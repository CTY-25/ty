...
abstract_text = (
    ...
    'Seaborn和Plotly等工具进行数据处理与可视化呈现，通过时间维度分析揭示了工作日与周末的'
...
add_paragraph(
    ...
    'Citi Bike系统为例，该系统在纽约市拥有超过2000个站点和25000辆自行车，日均骑行次数'
    '超过10万次，积累了海量的用户出行数据。'
)
add_paragraph(
...
add_paragraph(
    '本研究使用的数据集来源于美国纽约市Citi Bike共享单车系统的公开数据。Citi Bike是'
    '美国最大的共享单车运营系统之一，自2013年运营以来，通过其官方网站定期发布匿名化的'
    '出行数据记录。数据集以CSV（逗号分隔值）格式提供，每条记录代表一次完整的骑行行程。'
)
add_paragraph(
    '本研究选取2024年1月至2024年12月的月度数据作为分析样本，数据集总大小约为2.5GB，'
    '共包含超过1800万条骑行记录。数据字段主要包括：骑行ID（ride_id）、骑行类型'
    ...
    '用户类型（member_casual，会员或临时用户）等13个字段。'
)

add_pic_placeholder('Citi Bike数据下载页面', Inches(4.5), Inches(2.5))
add_figure_caption('图2-1 Citi Bike数据下载页面（示意图）')

add_paragraph(
    '数据采集的具体步骤如下：首先，访问Citi Bike官方网站的数据发布页面'
    '（https://citibikenyc.com/system-data），浏览2024年度各月的数据文件列表；'
    '其次，使用Python的requests库编写自动化下载脚本，批量下载12个月的CSV数据文件到本地'
    '存储；最后，使用Pandas库的read_csv函数将每个CSV文件加载为DataFrame对象，并使用'
    'pd.concat函数将12个月的数据合并为一个完整的数据集。'
)
...
add_paragraph(
    '第一步：数据整合。将12个月的CSV文件读取为Pandas DataFrame，使用pd.concat函数'
    '按行合并为一个统一的DataFrame对象，共包含18,745,632条记录和13个字段。检查合并后'
    '数据的基本信息，确认数据类型是否正确。'
)
...
add_paragraph(
    ...
    '值在纽约市地理范围内（纬度40.5°~41.0°N，经度-74.3°~-73.6°W）。'
)
...
add_paragraph(
    '经过上述预处理步骤，获得了一个包含18,220,000条记录、20个字段的高质量分析数据集，'
...
add_paragraph(
    '首先对骑行时长进行集中趋势分析。统计结果显示，Citi Bike用户单次骑行的平均时长为'
...
add_paragraph(
    ...
    '站点集中在曼哈顿中城和下城区域，其中"W 21 St & 6 Ave"站点日均出发骑行次数达385次，'
...
add_paragraph(
    ...
    '分布在曼哈顿的商业中心和交通枢纽。运营商应针对这些高流量站点建立动态调度机制，'
    '并考虑在周边增设扩展站点以缓解供需矛盾。'
)
add_paragraph(
...
add_paragraph(
    '（3）空间分析模块（Spatial Analysis）：基于站点地理坐标数据，使用Plotly的'
    'scatter_mapbox在地图上直观呈现站点分布和骑行流量。用户可按站点流量筛选Top N站点，'
...
add_paragraph(
    ...
    '（数据处理）、Plotly 5.17（交互式图表）、Scikit-learn 1.3（机器学习）和'
    'GeoPandas 0.14（地理数据处理）。开发环境为VS Code，使用Git进行版本控制，'
    '应用部署在Streamlit Community Cloud平台。'
)
add_paragraph(
    '开发过程的关键步骤包括：'
)
...
add_paragraph(
    ...
    '对比柱状图、分布直方图、地理散点图、饼图和热力图等。使用Plotly Express快速'
    '生成交互式图表，并通过Plotly Graph Objects进行精细化定制。'
)
add_paragraph(
...
add_paragraph(
    '本研究以纽约市Citi Bike共享单车系统2024年度的出行数据为研究对象，综合运用数据'
...
add_paragraph(
    '（1）数据源局限性。本研究仅使用了Citi Bike单一系统的数据，未来可整合多个城市共享单车'
...
references = [
    '[1] City of New York. Citi Bike System Data [EB/OL]. https://citibikenyc.com/system-data, 2024.',
