...
add_paragraph(
    '步骤二：数据加载与自动生成。编写数据加载模块，利用Streamlit的@st.cache_data'
    '装饰器实现数据缓存，避免每次交互都重新计算。应用内置完整的数据生成逻辑'
    '（30个上海核心站点、10万条骑行记录），当检测到本地无数据文件时自动生成测试数据，'
    '无需额外运行预处理脚本，确保本地开发与云部署均可直接运行。'
)
